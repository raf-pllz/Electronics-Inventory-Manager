...
def ErrorMSG(mode, filename):
    ...
    elif mode == "error-database-not-found":
        return f"- {ErrorTXT} There wasn\'t {bcolors.PURPLE}Database{bcolors.WARNING} found!{bcolors.ENDC}"
    elif mode == "error-item-name-already-exists":
        return f"- {ErrorTXT} The item name {bcolors.PURPLE}{filename}{bcolors.ENDC}{bcolors.FAIL} already exists{bcolors.ENDC}"
    elif mode == "error-item-id-already-exists":
        return f"- {ErrorTXT} The item ID {bcolors.PURPLE}{filename}{bcolors.ENDC}{bcolors.FAIL} already exists{bcolors.ENDC}"
    ...
    elif mode == "notice-default-directory-created":
    ...
    elif mode == "notice-obj-version":
        return f"- {NoticeTXT} The item you are currently viewing {bcolors.FAIL}might not be compatible with the current version of the software {bcolors.ENDC}({bcolors.PURPLE}Version : {Info.VERSION}{bcolors.ENDC}). Always take backups before you proceed."